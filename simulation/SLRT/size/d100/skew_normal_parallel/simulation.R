...
for(i in 4:length(sample_size)) {
  ...
  if(i < 4){
    ...
    z_set1 <- array(rsn(n*nreps*(gamma.dim - 1)/5, xi = location, omega = scale, alpha = shape), dim = c(n, gamma.dim - 1, nreps/5))
    z_set2 <- array(rsn(n*nreps*(gamma.dim - 1)/5, xi = location, omega = scale, alpha = shape), dim = c(n, gamma.dim - 1, nreps/5))
    z_set3 <- array(rsn(n*nreps*(gamma.dim - 1)/5, xi = location, omega = scale, alpha = shape), dim = c(n, gamma.dim - 1, nreps/5))
    z_set4 <- array(rsn(n*nreps*(gamma.dim - 1)/5, xi = location, omega = scale, alpha = shape), dim = c(n, gamma.dim - 1, nreps/5))
    z_set5 <- array(rsn(n*nreps*(gamma.dim - 1)/5, xi = location, omega = scale, alpha = shape), dim = c(n, gamma.dim - 1, nreps/5))
    z_set <- abind(z_set1, z_set2, z_set3, z_set4, z_set5, along = 3)
    rm(z_set1, z_set2, z_set3, z_set4, z_set5)
    gc();  gc();
    z_set <- abind(intercept_set, z_set, along = 2)
    rm(intercept_set)
    gc();  gc();
  ...
  for(iter in 1:nreps){
    ...
    y <- y_set[[iter]]
    print(iter)
    out <- SLRT_parallel(x, d, z, y, p, ninits = 10, ninits.equal = 10, n.core = 4)
