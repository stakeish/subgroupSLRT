...
#' A function to conduct EM test with parallel computing
#' @param x confounding covariates used for the outcome equation
#' @param d treatment variables
#' @param z covariates used for subgroup classification including intercept
#' @param J the number of initial gammas
...
#' @param epsilon threshold value for EM algorithm
#' @param maxit the maximum number of iterations for EM algorithm
#' @param n.core the number of cores for parallel
#' @param which.parallel the parameter over which parallelization takes place
em_test_parallel <- function(x, d, z, y, J, c, K, xx, xy, n.init = 20, upper = 5, lower = -5, lower.zero = 0.05, epsilon = 1e-08, maxit = 500, n.core = detectCores() - 1, which.parallel = c("J", "n.init")) {
  ...
  if(which.parallel == "J") {
    ...
    out <- mclapply(1:J, function(j, n, x, d, z, y, gamma.init.set, q1, theta.init.set, epsilon, maxit, n.init, K, c, upper, lower, lower.zero, xx, xy) {

      gamma <- gamma.init.set[, j]
      out <- em_test_gamma(n, x, d, z, y, gamma, q1, theta.init.set, epsilon, maxit, n.init, K, c, upper, lower, lower.zero, xx, xy)
      out

    }, n = n, x = x, d = d, z = z, y = y, gamma.init.set = gamma.init.set, q1 = q1, theta.init.set = theta.init.set, epsilon = epsilon, maxit = maxit, n.init = n.init, K = K, c = c, upper = upper, lower = lower, lower.zero = lower.zero, xx = xx, xy = xy, mc.cores = n.core)

  } else if (which.parallel == "n.init") {

    out <- lapply(1:J, function(j, n, x, d, z, y, gamma.init.set, q1, theta.init.set, epsilon, maxit, n.init, K, c, upper, lower, lower.zero, xx, xy, n.core) {

      gamma <- gamma.init.set[, j]
      out <- em_test_gamma_parallel(n, x, d, z, y, gamma, q1, theta.init.set, epsilon, maxit, n.init, K, c, upper, lower, lower.zero, xx, xy, n.core)
      out

    }, n = n, x = x, d = d, z = z, y = y, gamma.init.set = gamma.init.set, q1 = q1, theta.init.set = theta.init.set, epsilon = epsilon, maxit = maxit, n.init = n.init, K = K, c = c, upper = upper, lower = lower, lower.zero = lower.zero, xx = xx, xy = xy, n.core = n.core)

  } else {
    print("no appropriate option is selected for which.parallel.")
  }
...
em_test_gamma_parallel <- function(n, x, d, z, y, gamma, q1, theta.init.set, epsilon, maxit, n.init, K, c, upper, lower, lower.zero, xx, xy, n.core) {
  ...
  out.mle <- mclapply(1:n.init, function(j, theta.init.set, n, x, d, z, y, gamma, q1, epsilon, maxit, xx, xy){

    theta.init <- theta.init.set[, j]
    EM_gamma_fixed_cpp(n, x, d, z, y, gamma, q1, theta.init, epsilon, maxit, xx, xy)

  }, theta.init.set = theta.init.set, n = n, x = x, d = d, z = z, y = y, gamma = gamma, q1 = q1, epsilon = epsilon, maxit = maxit, xx = xx, xy = xy, mc.cores = n.core)
  ...
  out.final.mle <- mclapply(1:(n.init+1), function(j, theta.init.set, n, x, d, z, y, gamma, q1, epsilon, maxit, xx, xy){

    theta.init <- theta.init.set[, j]
    EM_gamma_fixed_cpp(n, x, d, z, y, gamma, q1, theta.init, epsilon, maxit, xx, xy)

  }, theta.init.set = theta.init.set, n = n, x = x, d = d, z = z, y = y, gamma = gamma0, q1 = q1, epsilon = epsilon, maxit = maxit, xx = xx, xy = xy, mc.cores = n.core)
...
bootstrap_test_wo_parallel <- function(x, d, z, y, J, c, K, B, n.init = 20, upper = 5, lower = -5, lower.zero = 0.05, epsilon = 1e-08, maxit = 500, which.parallel, n.core = detectCores() - 1) {

  xx <- t(x) %*% x
  xy <- t(x) %*% y

  out.em <- em_test_parallel(x=x, d=d, z=z, y=y, J=J, c=c, K=K, xx=xx, xy=xy, which.parallel = which.parallel)
  #out.em0 <- em_test(x, d, z, y, J, c, K, xx, xy)
  ...
  em.stat.boot <- lapply(1:B, function(b, x, d, z, J, c, K, alpha.null, beta.null, sigma.null, n, xx, xy, which.parallel) {

    y <- x %*% alpha.null + d * beta.null + rnorm(n, 0, sigma.null)
    xy <- t(x) %*% y
    out.em.b <- em_test_parallel(x=x, d=d, z=z, y=y, J=J, c=c, K=K, xx=xx, xy=xy, which.parallel = which.parallel)
    #print(b)
    #print(out.em.b$diff.vec)
    out.em.b$em.stat


  }, x = x, d = d, z = z, J = J, c = c, K = K, alpha.null = alpha.null, beta.null = beta.null, sigma.null = sigma.null, n = n, xx = xx, xy = xy, which.parallel = which.parallel)
...
bootstrap_test_unparallel <- function(x, d, z, y, J, c, K, B, n.init = 20, upper = 5, lower = -5, lower.zero = 0.05, epsilon = 1e-08, maxit = 500) {

  xx <- t(x) %*% x
  xy <- t(x) %*% y

  out.em <- em_test(x=x, d=d, z=z, y=y, J=J, c=c, K=K, xx=xx, xy=xy)
  #out.em0 <- em_test(x, d, z, y, J, c, K, xx, xy)
  ...
  em.stat.boot <- lapply(1:B, function(b, x, d, z, J, c, K, alpha.null, beta.null, sigma.null, n, xx, xy) {
    ...
    y <- x %*% alpha.null + d * beta.null + rnorm(n, 0, sigma.null)
    xy <- t(x) %*% y
    out.em.b <- em_test(x=x, d=d, z=z, y=y, J=J, c=c, K=K, xx=xx, xy=xy)
    #print(b)
    #print(out.em.b$diff.vec)
    out.em.b$em.stat
